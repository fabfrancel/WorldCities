...
public class SeedController : ControllerBase
{
    ...
    public async Task<ActionResult> Import()
    {
        ...
        for (int nRow = 2; nRow <= nEndRow; nRow++)
        {
            ...
            // store the country in our lookup to retrieve it Id later on
        ...
        // create a lookup dicitionary
        // containing all the cities already existing
        // into the Database (it will be empty on first run
        Dictionary<(string Name, decimal Lat, decimal Lon, int CountryId), City> cities = _context.Cities
              .AsNoTracking()
              .ToDictionary(x => (
                  Name: x.Name,
                  Lat: x.Lat,
        ...
        // iterates throuth all rows skkiping the first one
        for (int nRow = 2; nRow <= nEndRow; nRow++)
        {
